#Challenge 3#
#Solved using crypto library#
#Solution with help from https://stackoverflow.com/questions/39074253/extract-publickey-from-privatekey-input-using-python#

import binascii
from Crypto.PublicKey import RSA

from base64 import b64decode

def string2int(my_str):
    return int(binascii.hexlify(my_str), 16)

def int2string(my_int):
    return binascii.unhexlify(format(my_int, "x").encode("utf-8")).decode("utf-8")

#private key#
pem_key = b''

#decode the key and assign it the value of keypriv#
key = b64decode(pem_key)
keyPriv = RSA.importKey(key)

#Extract needed information from each aspect of the key, including n, e, d, p, q
modulusN = keyPriv.n
pubExpE = keyPriv.e
priExpD = keyPriv.d
primeP = keyPriv.p
primeQ = keyPriv.q
#print n, e and d
print(modulusN)
print(pubExpE)
print(priExpD)




